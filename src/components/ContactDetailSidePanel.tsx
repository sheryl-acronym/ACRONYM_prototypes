...
interface ContactDetailSidePanelProps {
  contactId: string;
  contact: Contact;
  onClose: () => void;
}

export const ContactDetailSidePanel: React.FC<ContactDetailSidePanelProps> = ({
  contactId,
  contact,
  onClose,
}) => {
  ...
  const handleOpenFullPage = React.useCallback(() => {
    // Navigate to full contact detail page
    ...
    <>
      ...
      <div className="fixed top-0 right-0 z-50 h-full w-[620px] max-w-[90vw] bg-white border-l border-slate-200 animate-in slide-in-from-right duration-200 flex flex-col overflow-hidden shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto">
          <ContactDetailPage contact={contact} hideTopBar={true} />
        </div>
      </div>
    </>
  );
...
export default ContactDetailSidePanel;
